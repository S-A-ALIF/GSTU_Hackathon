  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-h-0 flex flex-col">
        <div className={`flex-1 flex flex-col min-h-0 transition-all duration-300 overflow-hidden ${isMobileExpanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 xs:max-h-none xs:opacity-100'}`}>
          <div className="flex-1 min-h-0 overflow-y-auto space-y-4 pb-4 hide-scrollbar">
          {navSections.map((section, sIdx) => (
            <div key={sIdx} className="flex flex-col gap-1">
              {/* Section Header */}
              {!isCollapsed && (
                <div className="px-4 py-1 text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center justify-between">
                  <span>{section.title}</span>
                  {section.icon}
                </div>
              )}
              {isCollapsed && (
                <div className="mx-auto h-[1px] w-8 bg-slate-800 my-1"></div>
              )}
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex flex-row items-center justify-between gap-3 px-4 py-2.5 rounded-xl font-semibold text-sm transition-all w-full ${
                      ...
                      <div className={`flex items-center gap-3 w-full ${isCollapsed ? 'justify-center' : 'justify-start'}`}>
                        <div className="relative shrink-0">
                          ...
                          {(item.count > 0 && isCollapsed) && (
                            <span className="absolute -top-1 -right-1 flex h-3 w-3 items-center justify-center rounded-full bg-red-500 shadow-sm ring-2 ring-slate-900 hidden"></span>
                          )}
                        </div>
                        <div className={`items-center gap-2 truncate ${isCollapsed ? 'hidden' : 'flex'}`}>
                          <span>{item.label}</span>
                        </div>
                      </div>
                      {/* Show the pill badge on the right border ONLY when expanded */}
                      {(item.count > 0 && !isCollapsed) && (
                        <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-red-500 px-1.5 text-[10px] font-bold text-white shadow-sm ring-1 ring-slate-900/10 shrink-0">
