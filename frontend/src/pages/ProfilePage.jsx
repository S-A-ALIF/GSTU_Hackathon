...
export default function ProfilePage({ inDashboard = false }) {
  ...
  const handleCropImage = async () => {
    try {
      ...
      // Try with maximum quality first
      let croppedBlob = await getCroppedImg(imageToCrop, croppedAreaPixels, 0, { horizontal: false, vertical: false }, 1.0);
      
      // If >= 5MB, re-crop with reduced quality (e.g., 0.7)
      const FIVE_MB = 5 * 1024 * 1024;
      if (croppedBlob.size >= FIVE_MB) {
        croppedBlob = await getCroppedImg(imageToCrop, croppedAreaPixels, 0, { horizontal: false, vertical: false }, 0.7);
      }
    ...
    <div className={inDashboard ? '' : 'min-h-screen bg-slate-50 dark:bg-slate-900'}>
      ...
      {imageToCrop && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl overflow-hidden w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
              <h3 className="font-bold text-lg text-slate-900 dark:text-white">Crop Profile Picture</h3>
              <button 
            ...
            <div className="relative w-full h-96 sm:h-[450px] bg-slate-900">
              <Cropper
                image={imageToCrop}
                crop={crop}
                zoom={zoom}
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-900/50">
              ...
              <div className="flex gap-3 justify-end">
                <button
                  type="button"
                  onClick={() => setImageToCrop(null)}
                  className="px-4 py-2 font-semibold text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={isCropping}
                  className={`bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded-xl shadow-lg shadow-blue-500/30 transition-all flex items-center justify-center gap-2 ${
                    isCropping ? 'opacity-70 cursor-not-allowed' : ''
                  }`}
                >
                  {isCropping ? (
                    <>
